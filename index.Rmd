...
R is an environment for data manipulation, analysis and visualisation built around the R programming language.

At a first glance it may appear like a huge overkill to learn a programming language just
for the sake of doing some data analysis like fitting a regression model. Other statistics
software like [Stata](https://www.stata.com/) and [SPSS](https://www.ibm.com/analytics/us/en/technology/spss/) offer graphical user interfaces where a wide range of analysis can be performed simply by selecting options from menus. In my personal experience, however, non-trivial analysis using those software packages eventually leads to writing some sort of script files (e.g. [Stata `.do` files](https://www.stata.com/manuals13/u16.pdf) or [SPSS syntax files](https://www.spss-tutorials.com/spss-syntax-editor-window/)) and for that you need to learn
those specific languages.


R is by no means the only language used for data science. Other languages with
...
R script files are simple text files used to store a sequence of R commands. Per convention R script files use the `.R` extension. While you can use the R console directly to execute commands, script files are more convenient because you can reproduce the steps of the programme, track the files with a VCS (e.g. [git](https://git-scm.com/)) and share the files with other persons.
...
you can take a look at some [guidelines](https://google.github.io/styleguide/Rguide.xml).
...
R consists of two parts: the base system and an ecosystem of packages contributed by the community. Apart from the core functionality available in base R, everything else in R is provided through packages.
